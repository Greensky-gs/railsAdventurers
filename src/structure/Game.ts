import { AttachmentBuilder, ButtonInteraction, CollectedInteraction, ComponentType, Interaction, InteractionCollector, Message, MessageCreateOptions, MessageEditOptions, RepliableInteraction, StringSelectMenuBuilder, TextChannel, User } from "discord.js";
import { deckPart, decks, gameOptions, placed } from "../typings/game";
import { destinationSentence, getAllTouchesWith, getCities, getCitiesTouching, getDestinations, getPaths, getPathsFor, getTouches } from '../utils/core';
import { button, capitalize, plurial, random, resize, row, sentencePack, setDeleteTimer, shuffleArray } from "../utils/toolbox";
...
export class Game {
    ...
    private getPlayer(resolvable: Interaction | User) {
        return this._players.find(x => x.user.id === (resolvable instanceof User ? resolvable.id : resolvable.user.id));
    }
    private async getWayToPlace(player: Player, path: rails[], interaction: ButtonInteraction | Message<true>, reply: Message<true>, simulator = player.simulator, tunnels = false): Promise<deckPart> {
        ...
        const choose = async() => {
            ...
            const rowBuilder = () => {
                ...
                return row(new StringSelectMenuBuilder()
                    ...
                    .setMaxValues(1)
                    .setOptions(
                        opts.map((opt) => ({ label: `${(opt.key === 'engine' || opt.count === 3) ? 'Locomotive' : 'Wagon'}`, value: `${opt.key}.${opt.count}`, description: `Utilisez ${opt.count} ${opt.key === 'engine' ? 'locomotive' : `wagon${opt.count === 1 ? '' : 's'} ${colorsData[opt.key]?.name}`}` }))
                    )
                )
            }
            await interaction[interaction instanceof Message ? 'edit' : 'editReply']({
                content: tunnels ? `Quelle carte voulez-vous utiliser pour la ${index + 1}e locomotive supplémentaire ?` : `Quelle carte voulez-vous utiliser pour le ${index + 1}e rail ?`,
            ...
            if (choice.find(x => x.key === datas[0])) choice.find(x => x.key === datas[0]).count += parseInt(datas[1])
    ...
    private async drawDests(interaction: ButtonInteraction) {
        ...
        if (this.decks.destinations.length < 3) {
            await interaction.editReply({
                    content: `Il ne reste que **${this.decks.destinations.length}** destination${plurial(this.decks.destinations)}, voulez-vous quand même faire une pioche, en sachant que vous devez en accepter au moins une ?`,
                components: [
                    row(button({
                        label: 'Oui',
                        style: 'Success',
        ...
        const drawed = new Array(3).fill(null).map(() => this.drawDest()).filter(x => !!x)
        if (drawed.length === 1) {
            ...
            components: [
                row(
                    new StringSelectMenuBuilder()
                        ...
                        .setOptions(
                            drawed.map(x => ({ label: destinationSentence(x), description: destinationSentence(x, true), value: x.toString() }))
                        )
                )
            ]
        }).catch(log4js.trace)
    ...
    private generateSelectors(available: pointType<'city'>[]) {
        if (available.length <= 25) {
            return [new StringSelectMenuBuilder().setCustomId(Ids.PickCity).setMaxValues(1).setOptions(
                available.map((opt) => ({ label: opt.name, description: `Ralliez la ville de ${opt.name}`, value: opt.id.toString() }))
            )]
        } else {
            const first = available.splice(0, 25);
            return [new StringSelectMenuBuilder().setCustomId(Ids.PickCity).setMaxValues(1).setOptions(
                first.map((opt) => ({ label: opt.name, description: `Ralliez la ville de ${opt.name}`, value: opt.id.toString() }))
            ),new StringSelectMenuBuilder().setCustomId(Ids.PickCityT).setMaxValues(1).setOptions(
                available.map((opt) => ({ label: opt.name, description: `Ralliez la ville de ${opt.name}`, value: opt.id.toString() }))
            )]
        }
    }
    private listen() {
        ...
        this.collector.on('collect', async(interaction) => {
            ...
            if (interaction.customId === Ids.Place) {
                ...
                const available = cities.filter((x) => getPathsFor(x.id).filter(y => !this._placed.find(z => z.rail === y.id)).length > 0);
                ...
                const reply = await interaction.deferReply({ ephemeral: true, fetchReply: true }).catch(log4js.trace) as Message<true>;
                ...
                const surrounding = getCitiesTouching(getAllTouchesWith(firstCity.id)).filter(x => x.id !== firstCity.id);
                ...
                const paths = getTouches().filter(x => (x.from === firstCity.id && x.to === secondCity.id) || (x.from === secondCity.id && x.to === firstCity.id)).map(y => getPaths().find(x => x.id === y.by));
                ...
                if (path.paths.length > 1) {
                    await interaction.editReply({
                        ...
                        components: [
                            row(
                                ...path.paths.map((x, i) => button({ label: x[0].color === 'any' ? "Gris" : capitalize(colorsData[x[0].color].name), style: 'Secondary', custom: i.toString() }))
                            )
                        ]
                    }).catch(log4js.trace)
                    const rep = await waitForInteraction({
                ...
                if (selectedPath[0].types === 'tunnel') {
                    ...
                    const rep = await interaction.followUp({
                        fetchReply: true,
                        ephemeral: false,
                        content: `Vous allez construire le tunnel entre **${firstCity.name}** et **${secondCity.name}** avec ${sentencePack(way)}, voici vos trois cartes :`,
                    ...
                    const hasEnough = (simulator.maximumEnginesBuild().map(x => x.engines).reduce((a, b) => a + b, 0) + simulator.wagons.engine) >= toPay;
                    ...
                    const penalities = await this.getWayToPlace(player, new Array(toPay).fill(null).map(x => ({ color: 'any', requiresEngine: true, pos: [0, 0, 0, 0, 0, 0, 0, 0], types: 'tunnel' })), interaction, rep, simulator, true);
                    if (!penalities) {
                        retreat();
                ...
                if (res === 'last round') {
                    ...
                    this.channel.send({
                        ...
                        content: `${interaction.user.username} n'a plus que **${player.wagons}** wagon${plurial(player.wagons)}\n**${this.turnsRemaining} tours restants**`
                    }).catch(log4js.trace)
                }
                this._placed.push({
    ...
    private async instanceOfSelection(interaction: ButtonInteraction, baseCollector: InteractionCollector<ButtonInteraction>) {
        ...
        const reply = await interaction.reply({
            ephemeral: true,
            fetchReply: true,
            content: `Choisissez au moins **3** destinations\n${player.destinations.map(dest => `- ${destinationSentence(dest)}`).join('\n')}`,
            components: [row(new StringSelectMenuBuilder()
                    .setCustomId(Ids.PickDestinationsSelector)
                    .setMinValues(3)
                    .setMaxValues(5)
                    .setOptions(player.destinations.map(dest => ({ label: destinationSentence(dest), description: resize(destinationSentence(dest, true)), value: dest.toString() })))
                )]
        }).catch(log4js.trace) as Message<true>;
        if (!reply) return;
    ...
    // Management
    private distributeCard<T extends cardType>(player: number, type: T, ...cards: (T extends 'wagon' ? wagonKey : number)[]) {
    ...
    private async end() {
        ...
        const datas = this._players.map(x => x.calculateEndPoints()).sort((a, b) => a.player.points + b.player.points);
        ...
        if (isEquality()) {
            const slice = equalisedPlayers().length;
            const highest = equalisedPlayers().sort((a, b) => a.table.sort((c, d) => c.length + d.length)[0].length + b.table.sort((c, d) => c.length + d.length)[0].length)
    ...
    private clearCollectors() {
        this.collectors.filter(x => !x.ended).forEach((collector) => {
            collector.stop();
        });
    }
    ...
    private async generateContent(): Promise<MessageEditOptions & MessageCreateOptions> {
        ...
        if (!this.started) return {
            files: [attach],
            components: [row(button({ label: 'Prendre les destinations', id: 'PickDestinations', style: 'Secondary' }))]
        }
        return {
            files: [attach],
