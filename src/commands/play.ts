...
export default new AmethystCommand({
    ...
    if (isPlaying(interaction.user)) return interaction.reply({
        embeds: [alreadyPlaying(interaction.user)],
        ephemeral: true
    }).catch(log4js.trace)
    if (matchmaked(interaction.user)) return interaction.reply({
    ...
    if (!player1 && !player2 && visibility === GameMode.Private) return interaction.reply({
        embeds: [ baseDenied(interaction.user).setDescription(`Vous n'avez spécifié aucun utilisateur`) ],
        ephemeral: true
    }).catch(log4js.trace);
    if (selected.length === 0 && visibility === GameMode.Private) return interaction.reply({
        embeds: [baseDenied(interaction.user).setTitle("Partie annulée").setDescription(`Les personnes que vous avez choisit ne sont pas éligibles`)]
    }).catch(log4js.trace);
    ...
    const embed = () => {
        return baseInfo(interaction.user).setTitle("Partie").setDescription(`Une partie des aventuriers du rail va  commencer.\nJoueurs :${list.map(ping).join(' ')}\nNombre maximum de joueurs : **${players}**\n\nDémarrage automatique <t:${Math.floor(endsAt / 1000)}:R>`)
    }
    const components = () => {
        return [
            row(
    ...
    collector.on('collect', async(ctx) => {
        ...
        if (ctx.customId === Ids.PlayCancel) {
            ...
            const res = await ctx.reply({
                ...
                components: [row(button({ label: 'Oui', id: 'Yes', style: 'Success' }), button({ label: 'Non', id: 'No', style: 'Danger' }))],
                fetchReply: true
            }).catch(log4js.trace) as Message<true>
            if (!res) return;
    ...
    collector.on('end', async(_c, reason) => {
        if (reason === 'start') {
            ...
            const specificated = (): typeof list => {
                ...
                const res = [interaction.user, player1, player2, ...template.filter(x => !selected.find(y => y.id === x.id))].filter(x => !!x)
                return res;
            }
            new Game({
                channel: interaction.channel as TextChannel,
                colors: shuffleArray((['black', 'purple', 'white'] as playerColor[]).splice(0, list.length)) as [playerColor, playerColor, playerColor?],
                users: (order === GameOrder.Random ? shuffleArray(list) : order === GameOrder.Acceptation ? list : specificated()) as [User, User, User?]
            });
        } else {
            interaction.editReply({
                embeds: [ canceled() ],
